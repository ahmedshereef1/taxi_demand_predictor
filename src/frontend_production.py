...
def load_shape_data_file() -> gpd.geodataframe.GeoDataFrame:
    ...
    Returns:
        GeoDataFrame: columns -> (OBJECTID Shape_Leng Shape_Area zone
...
def _load_batch_of_features_from_store(current_date: datetime) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: n_features + 2 columns:
            - `rides_previous_N_hours`
            - `rides_previous_{N-1}_hours`
            - ...
            - `rides_previous_1_hours`
            - `pickup_hour`
            - `pickup_location_id`
    """
...
def _load_predictions_from_store(
    ...
    Args:
        from_pickup_hour (datetime): min datetime (rounded hour) for which
            we want to get predictions

        to_pickup_hour (datetime): max datetime (rounded hour) for which
            we want to get predictions
    ...
    return load_predictions_from_store(
        from_pickup_hour,
        to_pickup_hour,
    )
...
if next_hour_predictions_ready:
    ...
    raise Exception(
        'Features are not available for the last 2 hours. '
...
with st.spinner(text="Preparing data to plot"):
    ...
    def pseudocolor(val, minval, maxval, startcolor, stopcolor):
        ...
        return tuple(
            int((b - a) * f + a)
            for (a, b) in zip(startcolor, stopcolor)
        )
    ...
    max_pred = df['color_scaling'].max()
    min_pred = df['color_scaling'].min()
...
with st.spinner(text="Plotting time-series data"):
    ...
    for row_id in row_indices[:n_to_plot]:
        ...
        st.plotly_chart(fig, use_container_width=True, theme="streamlit", width=1000)
