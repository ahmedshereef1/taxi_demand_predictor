...
from src.config import FEATURE_VIEW_NAME
...
def fetch_features_and_targets_from_store(
    ...
    # filter by pickup_hour range
    ts_data['pickup_hour'] = pd.to_datetime(ts_data['pickup_hour'], utc=True)
    ts_data = ts_data[(ts_data['pickup_hour'] >= from_date) & (ts_data['pickup_hour'] < to_date)]
...
def load_features_and_target(
    ...
    if local_path_features_and_target:
        ...
        from_date = pd.to_datetime(date.today() - timedelta(days=52*7), utc=True)
        to_date = pd.to_datetime(date.today(), utc=True)
